...
// ===============================
let records = [];
let currentOption = "Inventory Network";
...
function getCurrentPermission() {
    return localStorage.getItem("inventoryNetworkPermission") || "none";
}
...
function exportExcel() {
    window.open(`${API_BASE_URL}/api/inventory-network-records/export`,"_blank");
}
...
async function importExcel(input) {
    ...
    try {
        const response =
            await fetch(
                `${API_BASE_URL}/api/inventory-network-records/import`,
                {
                    method: "POST",
                    body: formData
                }
    ...
    filteredRecords.forEach(item => {
        html += `
            <tr>
                ...
                <td>${item.prNum || "-"}</td>
                <td>${item.poNum || "-"}</td>
                <td>${(item.networkDetails || []).map(x => x.code).join("<br><br>")}</td>
                <td>${(item.networkDetails || []).map(x => x.desc).join("<br><br>")}</td>
                <td>${(item.networkDetails || []).map(x => x.quantity).join("<br><br>")}</td>
...
async function loadRecords() {
    try {
        const response =
            await fetch(
                `${API_BASE_URL}/api/inventory-network-records`
            );
    ...
    try {
        ...
        if (editingRecordId) {
            response = await fetch(
                `${API_BASE_URL}/api/inventory-network-records/${editingRecordId}`,
                {
                    method: "PUT",
                    headers: {
...
async function deleteRecord(id) {
    ...
    try {
        const response =
            await fetch(
                `${API_BASE_URL}/api/inventory-network-records/${id}`,
                {
                    method: "DELETE"
                }
            );
        ...
        showToast("success", data.message || "Inventory Network Record Deleted Successfully");
...
function openModal() {
    ...
    if (countText) {
        countText.innerText = "Total Network Items Added: 0";
    }
}
...
function openAddModal() {
    ...
    openModal();
    // Network requires at least one row
    addMaterialRow();
}
...
async function addRecord(event) {
    ...
    const networkRows = document.querySelectorAll(".network-entry");

    if (networkRows.length === 0) {
        showToast("warning", "Please add at least one Network");
        return;
    }
...
function editRecord(id) {
    ...
    document.getElementById("serviceContainer").innerHTML = "";
    if (record.networkDetails) {
        record.networkDetails.forEach(network => {
            addMaterialRow(network);
        });
    }
}
...
function removeMaterialRow(button) {
    button.closest(".network-entry").remove();
...
function updateMaterialDeleteButtons() {
    const rows = document.querySelectorAll(".network-entry");
...
function updateMaterialCount() {
    const total = document.querySelectorAll(".network-entry").length;
    const countText = document.getElementById("serviceCountText");

    if (countText) {
        countText.innerText = `Total Network Items Added: ${total}`;
    }
}
