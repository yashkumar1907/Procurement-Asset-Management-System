...
function getCurrentPermission() {
    return localStorage.getItem("inventoryDepartmentPermission") || "none";
}
...
function exportExcel() {
    window.open(`${API_BASE_URL}/api/inventory-department-records/export`,"_blank");
}
...
async function importExcel(input) {
    ...
    try {
        const response =
            await fetch(
                `${API_BASE_URL}/api/inventory-department-records/import`,
                {
                    method: "POST",
                    body: formData
                }
...
async function loadRecords() {
    try {
        const response =
            await fetch(
                `${API_BASE_URL}/api/inventory-department-records`
            );
    ...
    try {
        ...
        if (editingRecordId) {
            response = await fetch(
                `${API_BASE_URL}/api/inventory-department-records/${editingRecordId}`,
                {
                    method: "PUT",
                    headers: {
...
async function deleteRecord(id) {
    ...
    try {
        const response =
            await fetch(
                `${API_BASE_URL}/api/inventory-department-records/${id}`,
                {
                    method: "DELETE"
                }
            );
        ...
        showToast("success", data.message || "Inventory Department Record Deleted Successfully");
...
function openModal() {
    ...
    if (countText) {
        countText.innerText = "Total Department Items Added: 0";
    }
}
...
function openAddModal() {
    ...
    openModal();
    // Department requires at least one row
    addMaterialRow();
}
...
async function addRecord(event) {
    ...
    const departmentRows = document.querySelectorAll(".department-entry");
    

    if (departmentRows.length === 0) {
        showToast("warning", "Please add at least one Department");
        return;
    }
...
function addMaterialRow(material = {}) {
    ...
    div.className = "department-entry";
...
function removeMaterialRow(button) {
    button.closest(".department-entry").remove();
...
function updateMaterialDeleteButtons() {
    const rows = document.querySelectorAll(".department-entry");
...
function updateMaterialCount() {
    const total = document.querySelectorAll(".department-entry").length;
    const countText = document.getElementById("serviceCountText");

    if (countText) {
        countText.innerText = `Total Department Items Added: ${total}`;
    }
}
