...
// ===============================
// CREATE RECORD (UPDATED)
// ===============================
router.post("/", async (req, res) => {
    try {
        const parsedNetworkDetails = req.body.networkDetails || [];
        ...
        res.status(201).json({
            message: "Inventory Network Record Added Successfully"
        });
    }
    catch (error) {
        console.error(error);
...
// ===============================
// GET ALL RECORDS (UPDATED)
// ===============================
router.get("/", async (req, res) => {
    try {
        const records = await InventoryNetworkRecord.find();
...
router.put("/:id", async (req, res) => {
    try {
        const existingRecord = await InventoryNetworkRecord.findById(req.params.id);
        if (!existingRecord) {
            return res.status(404).json({
                message: "Inventory Network Record not found"
            });
        }
        ...
        const updatedNetworks =
            req.body.networkDetails ||
            existingRecord.networkDetails;
...
router.delete("/:id", async (req, res) => {
    try {
        const record = await InventoryNetworkRecord.findById(req.params.id);
        if (!record) {
            return res.status(404).json({
        ...
        await InventoryNetworkRecord.findByIdAndDelete(req.params.id);

        res.status(200).json({
            message: "Inventory Network Record Deleted Successfully"
        });
    }
    catch (error) {
        console.error(error);
...
// ===============================
// EXPORT INVENTORY NETWORK EXCEL
// ===============================
router.get("/export", async (req, res) => {
    try {
        ...
        records.forEach(record => {
            record.networkDetails.forEach(network => {
                recordSheet.push({
                    ...
                    "Material Code": network.code,
                    "Material Description": network.desc,
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, "Inventory Network Records");
        ...
        res.setHeader(
            "Content-Disposition",
            "attachment; filename=Inventory_Network_Export.xlsx"
        );
...
// ===============================
// IMPORT INVENTORY NETWORK EXCEL
// ===============================
router.post("/import", excelUpload.single("excelFile"), async (req, res) => {
    try {
        ...
        for (const prNum of Object.keys(groupedRecords)) {
            const existingRecord = await InventoryNetworkRecord.findOne({prNum});
            ...
            const record =
                new InventoryNetworkRecord({
                    ...
                    networkDetails: recordData.networkDetails
                });
