...
router.post("/", upload.single("invoicePdf"), async (req, res) => {
    try {
        // Check PO
        const record = await ContractRecord.findById(req.body.recordId);
        if (!record) {
        ...
        res.status(201).json({
            message: "Contract Detail Added Successfully"
        });
    }
    catch(error) {
        console.log(error);
...
router.get("/:recordId", async (req, res) => {
    try {
        const details = await ContractDetail.find({recordId: req.params.recordId});
        res.status(200).json(details);
    }
    catch(error) {
        console.log(error);
...
router.delete("/:id", async (req, res) => {
    try {
        ...
        res.status(200).json({
            message: "Contract Detail Deleted Successfully"
        });
    }
    catch(error) {
        console.log(error);
