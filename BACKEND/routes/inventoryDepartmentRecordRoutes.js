...
router.post("/", async (req, res) => {
    try {
        const parsedDepartmentDetails = req.body.departmentDetails || [];

        const record =
            new InventoryDepartmentRecord({
                ...req.body,
                departmentDetails: parsedDepartmentDetails
            });

        await record.save();

        res.status(201).json({
            message: "Inventory Department Record Added Successfully"
        });
    }
    catch (error) {
        console.error(error);
...
router.get("/", async (req, res) => {
    try {
        const records = await InventoryDepartmentRecord.find();
...
router.put("/:id", async (req, res) => {
    try {
        const existingRecord = await InventoryDepartmentRecord.findById(req.params.id);
        if (!existingRecord) {
            return res.status(404).json({
                message: "Inventory Department Record not found"
            });
        }
        ...
        const updatedDepartments =
            req.body.departmentDetails ||
            existingRecord.departmentDetails;

        updateData.departmentDetails = updatedDepartments;

        await InventoryDepartmentRecord.findByIdAndUpdate(req.params.id, updateData);

        res.status(200).json({
            message: "Inventory Department Record Updated Successfully"
        });
    }
    catch (error) {
        console.error(error);
...
router.delete("/:id", async (req, res) => {
    try {
        const record = await InventoryDepartmentRecord.findById(req.params.id);
        if (!record) {
            return res.status(404).json({
        ...
        await InventoryDepartmentRecord.findByIdAndDelete(req.params.id);

        res.status(200).json({
            message: "Inventory Department Record Deleted Successfully"
        });
    }
    catch (error) {
        console.error(error);
...
// ===============================
// EXPORT INVENTORY Department EXCEL
// ===============================
router.get("/export", async (req, res) => {
    try {
        ...
        records.forEach(record => {
            record.departmentDetails.forEach(department => {
                recordSheet.push({
                    ...
                    "Material Code": department.code,
                    "Material Description": department.desc,
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, "Inventory Department Records");
        ...
        res.setHeader(
            "Content-Disposition",
            "attachment; filename=Inventory_Department_Export.xlsx"
        );
...
// ===============================
// IMPORT INVENTORY DEPARTMENT EXCEL
// ===============================
router.post("/import", excelUpload.single("excelFile"), async (req, res) => {
    try {
        ...
        const recordSheet = XLSX.utils.sheet_to_json(workbook.Sheets["Inventory Department Records"]);
        ...
        recordSheet.forEach(row => {
            ...
            groupedRecords[prNum].departmentDetails.push({
        ...
        for (const prNum of Object.keys(groupedRecords)) {
            const existingRecord = await InventoryDepartmentRecord.findOne({prNum});
            ...
            const record =
                new InventoryDepartmentRecord({
                    ...
                    departmentDetails: recordData.departmentDetails
                });
